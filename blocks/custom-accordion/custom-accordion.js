...
function normalizeCells(cells, rowheaderRole = 'rowheader', cellRole = 'cell') {
  [...cells].forEach((cell, j) => {
    ...
    if (
      cell.lastChild
      && cell.lastChild.nodeType === 3
      && cell.lastChild.nodeValue.trim() === ''
...
function activateMobileColumn(block, index) {
  block
    .querySelectorAll('.cell.expand')
    .forEach((cell) => cell.classList.remove('expand'));
  block
    .querySelectorAll(
      `.image-header .cell:nth-child(${index}),.row .cell:nth-child(${
        index + 1
      })`,
    )
  ...
  block
    .querySelectorAll('[role="rowgroup"][aria-hidden="false"]')
    .forEach((rowgroup) => {
...
export default async function decorate(block) {
  ...
  if (header.children.length > 1) {
    ...
    .map(
      (columnHeader, i) => `<option value="${i + 1}">${columnHeader.textContent}</option>`,
    )
    .join('')}
          </select>`;
    mobileColumnHeader.firstElementChild.addEventListener(
      'change',
      changeMobileColumn,
    );
  ...
  for (let i = firstRowIndex, rowgroup = null; i < rows.length; i += 1) {
    ...
    const firstChild = (cells[0].firstElementChild
        && cells[0].firstElementChild.firstElementChild)
      || cells[0].firstElementChild;
    const isWrappedByStrong = cells[0].children.length === 1
      && firstChild
